...
PostgreSQL sudah disiapkan dan adapter runtime backend sudah menggunakan PostgreSQL.

- Adapter aktif saat ini: `src/db.py` menggunakan PostgreSQL.
- Import awal Spreadsheet: `src/migrate_spreadsheet.py`.
...
- Database default: `botshopee`.
- PostgreSQL production: `168.144.143.203:5555`.

Runtime worker membaca PostgreSQL. Spreadsheet hanya dibaca oleh migration script satu kali; jangan mengembalikan pembacaan Spreadsheet ke dalam siklus worker.
...
```text
postgresql://botshopee:<PASSWORD>@localhost:5434/botshopee
```

Import Spreadsheet na PostgreSQL:
...
```text
postgresql://botshopee:<PASSWORD>@168.144.143.203:5555/botshopee?sslmode=require
```
...
| M Nama outlet | `outlets.long_name` |
| N Nama pendek outlet | Tidak digunakan; gunakan nama outlet utama |
...
8. Adapter PostgreSQL adalah runtime adapter; tambahkan test integrasi PostgreSQL.
